...
class DockerWorker:
    ...
    def __init__(self, worker_id=None, master_host=None, master_port=None):
        # Worker配置
        self.worker_id = worker_id or f"docker-worker-{os.uname().nodename}-{int(time.time())}"
    ...
    def start(self):
        ...
        logger.info(f"启动Docker Worker: {self.worker_id}")

        # 初始化GPU监控
        self.gpu_monitor.start()

        # 初始化任务执行器
        self.task_executor = TaskExecutorFactory.create_executor("docker", self.gpu_monitor)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Docker GPU Worker")
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('docker-worker.log'),
